...
(function (exports) {
  ...
  function processBoilerplateHtml(stream) {
    new Sink(stream).on('data', function(buffer) {
      ...
      // add a development-mode require.js script version of main.js and comment out the existing production-mode
      // script tag
  ...
  exports.template = function (grunt, init, done) {
    ...
    request('http://html5boilerplate.com/', function (error, response, body) {
      if (!error && response.statusCode === 200) {
        var matches = /<a[^>]+href="?([^" >]+archive[^"> ]*)/.exec(body);
        if (matches) {
          var url = matches[1];
      ...
      grunt.fail.warn('Couldn\'t determine the latest version of HTML5Boilerplate: ' + JSON.stringify(error));
      done();
    });
  };
